...
function tave_send($contactForm) {
  ...
  if ($sendTave == "checked") {
  	curl_setopt_array($curlHandle, array(CURLOPT_HTTPHEADER => array('X-Tave-No-Email-Notification: true')));
  }
...
<div class="wrap">
	...
	<form method="post" action="options.php">
		...
		<table class="form-table">
			...
			<td><input type="checkbox" name="send-CF7" value="1" <?php checked( 1 == get_option( 'send-CF7' ) ); ?> />Check this box to stop receiving the Contact Form 7 email.</td>
			</tr>
			<tr valign="top">
			<th scope="row">Don't Send T&aacute;ve email?</th>
			<td><input type="checkbox" name="send-Tave" value="1" <?php checked( 1 == get_option( 'send-Tave' ) ); ?> />Check this box to stop receiving the T&aacute;ve email.</td>
			</tr>
		</table>
		<?php submit_button(); ?>
	</form>
