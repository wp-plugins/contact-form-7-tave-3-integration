...
function tave_wpcf7_register_mysettings() {
  ...
  register_setting( 'tave_wpcf7-settings-group', 'tave-send-form' );
}

function tave_wpcf7_settings_page() {

	if ( get_option('tave-send-form') == true ) { $display = 'checked'; }
	else { $display = ''; }
	update_option( 'tave-send-form', $display );
?>
...
<div class="wrap">
	...
	<form method="post" action="options.php">
		...
		<table class="form-table">
			...
			<td><input type="checkbox" name="tave-send-form" <?php echo get_option('tave-send-form'); ?> /><br>Check this box to only receive the T&aacute;ve email and not the contact form 7 generated one.</td>
			</tr>
		</table>
		<?php submit_button(); ?>
	</form>
	<p>Plugin is the brainchild of <a href="http://www.rowellphoto.com/">Ryan Rowell</a>, and much thanks is given to Jason Pirkey of <a href="http://tave.com">T&aacute;ve</a> for helping interpret the API, Without him this plugin wouldn't work.</p>
	</div>
